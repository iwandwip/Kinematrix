...
//                set(config_data_ptr_->header.pin_ptr_[j + (size / 2)], HIGH);
    ...
    void BaseChannel::taskSequence16DualSideSequential() {
        if (config_data_ptr_->header.pin_size_ < 6) return;
        off();
        ...
        for (int i = 0; i < halfSize - 1; i++) {
            set(config_data_ptr_->header.pin_ptr_[i], HIGH);
            set(config_data_ptr_->header.pin_ptr_[i + 1], HIGH);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 2 - i], HIGH);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 1 - i], HIGH);
            ...
            set(config_data_ptr_->header.pin_ptr_[i], LOW);
            set(config_data_ptr_->header.pin_ptr_[i + 1], LOW);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 2 - i], LOW);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 1 - i], LOW);
        }

        for (int i = halfSize - 3; i >= 0; i--) {
            set(config_data_ptr_->header.pin_ptr_[i], HIGH);
            set(config_data_ptr_->header.pin_ptr_[i + 1], HIGH);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 2 - i], HIGH);
            set(config_data_ptr_->header.pin_ptr_[config_data_ptr_->header.pin_size_ - 1 - i], HIGH);
    ...
    void BaseChannel::taskSequence17MirrorPattern() {
        if (config_data_ptr_->header.pin_size_ < 4) return;
        off();
        ...
        for (int i = 0; i < halfSize; i++) {
            set(config_data_ptr_->header.pin_ptr_[i], HIGH);
            set(config_data_ptr_->header.pin_ptr_[i + halfSize], HIGH);
            sleep(channel_data_.delay_time_ * 2);
            set(config_data_ptr_->header.pin_ptr_[i], LOW);
            set(config_data_ptr_->header.pin_ptr_[i + halfSize], LOW);
        }

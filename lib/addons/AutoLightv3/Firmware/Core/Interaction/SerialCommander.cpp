...
namespace AutoLight {
    ...
    SerialCommander::SerialCommander() :
            led_(nullptr), config_(nullptr), button_(nullptr) {
    ...
    void SerialCommander::init(BaseChannel *led, BaseConfig *config, ButtonInterrupt *button) {
        led_ = led;
        config_ = config;
        button_ = button;
        loadWiFiPreferences();
    ...
    void SerialCommander::initWiFi() {
        ...
        autolight_wifi_mode_t mode = config_mgr.getWiFiMode();
        
        if (mode == AUTOLIGHT_WIFI_MODE_STA) {
            startStationMode();
        } else {
            startAccessPoint();
        }
    ...
    void SerialCommander::loadWiFiPreferences() {
        ...
        if (config_mgr.loadFromPreferences()) {
            Credentials creds = config_mgr.getCredentials();
            strcpy(ap_config_.ssid, creds.ssid.c_str());
            strcpy(ap_config_.password, creds.password.c_str());
            ap_config_.is_loaded_from_preferences = true;
            Serial.println("✓ WiFi config loaded from ConfigManager");
        } else {
            strcpy(ap_config_.ssid, "AutoLightV3");
            strcpy(ap_config_.password, "");
            ap_config_.is_loaded_from_preferences = false;
            Serial.println("ℹ Using default WiFi config, initializing ConfigManager");
            config_mgr.initializeDefaults();
        }
    }
    ...
    void SerialCommander::startStationMode() {
        WiFi.mode(WIFI_STA);
        connectToWiFi();
    }

    void SerialCommander::connectToWiFi() {
        Serial.printf("🔗 Connecting to WiFi: '%s'...\n", ap_config_.ssid);
        
        if (strlen(ap_config_.password) == 0) {
            WiFi.begin(ap_config_.ssid);
        } else {
            WiFi.begin(ap_config_.ssid, ap_config_.password);
        }
    ...
    void SerialCommander::wifi(String args) {
        if (args.length() == 0) {
            ...
            if (mode == "ap") {
                ...
                ConfigManager::getInstance().updateWiFiMode(AUTOLIGHT_WIFI_MODE_STA);
                Serial.println("✓ WiFi mode set to STA");
                Serial.println("  Use 'wifi restart' to apply changes");
            } else {
                Serial.println("❌ Invalid mode. Use: ap, sta");
            }
            ...
            Serial.println("Usage: wifi, wifi set \"ssid\" \"pass\", wifi mode ap/sta, wifi restart, wifi reset, wifi clients");
        }
    }
    ...
    void SerialCommander::help(String args) {
        ...
        if (current_mode_ >= SERIAL_READ_ONLY) {
            ...
            Serial.println("  wifi                    - Show current WiFi status");
            Serial.println("  wifi set \"ssid\" \"pass\" - Set WiFi credentials (for AP or STA)");
            Serial.println("  wifi mode ap           - Set to Access Point mode");
            Serial.println("  wifi mode sta          - Set to Station mode");
            Serial.println("  wifi restart           - Restart WiFi with current settings");
            Serial.println("  wifi reset             - Reset to default settings (AP mode)");
            Serial.println("  wifi clients           - Show connected clients (AP mode only)\n");
        ...
        Serial.println("  wifi set \"MyAP\" \"\"     - Create open AP/STA");
        Serial.println("  wifi mode sta          - Switch to Station mode");
        Serial.println("=====================================");
    }
    ...
    void SerialCommander::showWiFiInfo() {
        ConfigManager &config_mgr = ConfigManager::getInstance();
        autolight_wifi_mode_t current_mode = config_mgr.getWiFiMode();
        
        Serial.println("📶 WiFi Status:");
        Serial.printf("  Mode: %s\n", current_mode == AUTOLIGHT_WIFI_MODE_AP ? "Access Point (AP)" : "Station (STA)");
        Serial.printf("  SSID: %s\n", ap_config_.ssid);
        Serial.printf("  Security: %s\n", strlen(ap_config_.password) == 0 ? "Open" : "WPA2");
        
        if (current_mode == AUTOLIGHT_WIFI_MODE_AP) {
    ...
    void SerialCommander::restartAP() {
        ...
        Serial.printf("🔄 Restarting WiFi in %s mode...\n", 
                      current_mode == AUTOLIGHT_WIFI_MODE_AP ? "AP" : "STA");
        ...
        if (current_mode == AUTOLIGHT_WIFI_MODE_STA) {
            startStationMode();
        } else {
            startAccessPoint();
        }
    ...
    void SerialCommander::resetToDefaults() {
        ...
        Serial.println("✓ Reset to default WiFi settings (AP mode)");
    ...
    bool SerialCommander::isCommandAllowed(const String &command_type) {
        ...
        if (command_type == "wifi" || command_type == "seq list" || command_type == "serial") {

...
#define ENABLE_ADDONS_AUTOLIGHT_V1
...
const int INTERRUPT_PIN = 36;    // [DEFAULT] 36     | Pin number to handle an interrupt.
...
void setup() {
  ...
  button.attach(INTERRUPT_PIN, INPUT_PULLUP, FALLING, []() [[IRAM_ATTR]] -> void {
    button.readInterrupt([]() -> void {
      led.changeMode();
    });
  });
  button.debug();
